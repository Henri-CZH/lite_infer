...
class CLIPVisionEmbeddings(nn.Module):
    ...
    def forward(self, pixel_values: torch.Tensor) -> torch.Tensor:
        ...
        patch_embeds = self.patch_embedding(pixel_values.to(dtype=target_dtype))  # (batch_size, embedding_dim, num_patches, num_patches)
        patch_embeds = patch_embeds.flatten(2).transpose(1, 2) # (batch_size, num_patches**2, embedding_dim)

        class_embeds = self.class_embedding.expand(batch_size, 1, -1)
        embeddings = torch.cat([class_embeds, patch_embeds], dim=1)
        embeddings = embeddings + self.position_embedding(self.position_ids) # (batch_size, num_patches**2 + 1, embedding_dim)
...
class CLIPAttention(nn.Module):
    ...
    def forward(
        ...
        """Input shape: Batch x Time x embedding_dim"""
        ...
        attn_output = self.out_proj(attn_output) # (batch_size, tgt_len, embedding_dim)
...
class CLIPMLP(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        hidden_states = self.fc2(hidden_states) # (batch_size, tgt_len, embedding_dim)
...
class CLIPEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        hidden_states = residual + hidden_states # (batch, seq_len, embed_dim)
