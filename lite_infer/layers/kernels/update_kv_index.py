...
def _fwd_kernel_update_kv_index(
    ...
    b_req_idx,            # decode_batch 批次中每个请求的 ID，形状为 (num_requests,)
    b_seq_len,            # decode_batch 中每个请求的序列长度，形状为 (num_requests,)
    select_index,             # decode_batch 中每个 tokens的 KV 索引，形状为 (num_requests,)
    stride_req_to_token_b,  # req_to_token_indexs 在第一个维度（序列长度）的步幅
    stride_req_to_token_s   # req_to_token_indexs 在第二个维度（请求）的步幅
):
    # 获取当前程序的 ID，即线程的索引
...
def update_kv_index(req_to_token_indexs, b_req_idx, b_seq_len, select_index):
    ...
    参数：
        ...
        b_req_idx (torch.Tensor): 批次中每个请求的 ID, 形状为 (num_requests,)。
        b_seq_len (torch.Tensor): 每个请求的序列长度，形状为 (num_requests,)。
        select_index (torch.Tensor): 每个令牌的 KV 索引，形状为 (num_requests,)。
