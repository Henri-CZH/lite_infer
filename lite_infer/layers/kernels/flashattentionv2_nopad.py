...
# 根据key['B_Seqlen', 'HEAD_DIM'] 参数, 进行BLOCK_M_SIZE, BLOCK_N_SIZE的调优
@triton.autotune(
...
def flash_attentionv2_nopad_kernel(
    ...
    BLOCK_M_SIZE: tl.constexpr, # BLOCK size of m_size dimension，即 Q 矩阵行数分成了m_size // BLOCK_M_SIZE 块，块大小是 BLOCK_M_SIZE, 由 autotune 自动选择
    BLOCK_N_SIZE: tl.constexpr, # BLOCK size of n_size dimension, 由 autotune 自动选择
):
    """
...
def flash_attentionv2_no_pad(
    ...
    grid = lambda meta: (triton.cdiv(max_seq_len, meta["BLOCK_M_SIZE"]), batchs * n_heads, 1)

    flash_attentionv2_nopad_kernel[grid](
        q,
        k,
        v, 
        output,
...
# 内核精度验证与性能比较函数封装
# =============================================================================
def run_flash_attentionv2_no_pad_benchmark(batch=4, n_heads=32, head_dim=128, max_seq_len_list=[1024, 2048, 4096]):
    """
    构造输入 q/k/v 张量形状为 [batch, n_heads, head_dim] (q) 和 [max_seq_len, n_heads, head_dim] (k, v),
    验证 flash_attentionv2_no_pad 输出结果与标准 attention 对齐（允许一定误差），
    ...
    triton_output = flash_attentionv2_no_pad(q, k, v, sm_scale, b_start_loc, b_seq_len, max_seq_len)
    ...
    for seq_len in max_seq_len_list:
        ...
        # 预热
        _ = flash_attentionv2_no_pad(q, k, v, sm_scale, b_start_loc, b_seq_len, seq_len)
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("./images/flashattentionv2_nopad_benchamrk.png")
...
if __name__ == "__main__":
    stats = run_flash_attentionv2_no_pad_benchmark()
    print("Benchmark statistics:", stats)


# max_seq_len = 1024: flash_attn = 1.386 ms, standard_attn = 3.345 ms
# max_seq_len = 2048: flash_attn = 5.083 ms, standard_attn = 7.459 ms
# max_seq_len = 4096: flash_attn = 19.644 ms, standard_attn = 26.524 ms
# best config selected: BLOCK_M_SIZE: 128, BLOCK_N_SIZE: 32, num_warps: 4, num_ctas: 1, num_stages: 2