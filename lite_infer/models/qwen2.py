...
class Qwen2Attention(nn.Module):
    ...
    def forward(
        ...
        mask_nan = torch.isnan(attn_output)
        attn_output = torch.where(mask_nan, 1e-8, attn_output)
        mask_nan = torch.isinf(attn_output)
        attn_output = torch.where(mask_nan, 1e-8, attn_output)
        if torch.isnan(attn_output).any() or torch.isinf(attn_output).any(): # 检查 NaNs and inf
...
class Qwen2Model(nn.Module):
    ...
    def forward(
        ...
        output = F.linear(h, self.lm_head_weight.data) # [batch_size, seq_len, hidden_dim]@(hidden_dim, vocab_size)=[batch_size, seq_len, vocab_size]
        return output
