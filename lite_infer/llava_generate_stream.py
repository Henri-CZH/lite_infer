...
class LlavaGeneratorStream:
    ...
    def generate_stream(
        ...
        max_prompt_len = max(len(t) for t in prompt_tokens) # max prompt length within a batch
        assert max_prompt_len <= self.max_seq_len
        total_seq_len = min(self.max_seq_len, max_gen_len + max_prompt_len)
        actual_prompt_lens = torch.tensor([len(t) for t in prompt_tokens], dtype=torch.long, device=self.device) # seq len of each request
        pad_id = self.tokenizer.pad_token_id if self.tokenizer.pad_token_id is not None else self.tokenizer.eos_token_id # get id of padding token 
        ...
        # 初始化每个样本已输出的位置, if echo==false->生成的 token 将从提示词之后开始 pos=len(prompt_tokens[bsz_id]), otherwise->生成的token包含提示词部分(即从序列开头开始, pos=0)
        last_yielded_pos = [len(prompt_tokens[i]) if not echo else 0 for i in range(bsz)]
        ...
        # prefill_select_index shape: (bsz * max_prompt_seq_len + (number_patchs * number_patchs - 1) * img_batch_size, )
        ...
        for cur_pos in range(max_prompt_len, total_seq_len):
            ...
            logits = self.model_executor.forward(input_ids, position_ids, image_tensors) # step 0: position_ids 由 llava 模型类给出 # [batch_size, seq_len, vocab_size]
