...
from lite_infer.generate import GenerateText
...
def compare_inference_speed(
    ...
    torch.cuda.empty_cache() # 使用完成后释放 hf 占用的显存
...
def main():
    ...
    hf_model_name = "/home/my_ubuntu/AI_deployment/lite_llama/model/Llama-3.2-1B-Instruct" # HF model path
    custom_checkpoints_dir = "/home/my_ubuntu/AI_deployment/lite_llama/my_weight/Llama-3.2-1B-Instruct"  # 通过lite llama框架转换的模型
...
# lite_llama inference time: 86.0956 s
# Transformers inference time: 124.6410 s
# lite_llama inference output tokens number: 30408
# Transformers inference output tokens number: 30175
# lite_llama throughput: 353.19 tokens/s
# Transformers throughput: 242.10 tokens/s
# lite_llama per token latency: 2.831346 ms/token
# Transformers per token latency: 4.130605 ms/token